...
import { useGetAdminCourses } from 'src/hooks/api/useAdminCourses.js';
...
const CoursesTable = () => {
  ...
  const { data, isFetching } = useGetAdminCourses(params);
  const currentPageData = data?.data;
  const courses = currentPageData?.content || [];
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {isFetching ? (
              ...
              courses?.map((row) => <Row row={row} key={row.id} />)
            )}
          </TableBody>
        </Table>
      </TableContainer>
