...
const MenuTable = ({ filterApplied }) => {
  ...
  const onClickSave = () => {
    ...
    const formattedBody = editedPurchaseList?.map((purchase) => {
      return {
        ...
        totalKiloGlobal: purchase?.totalKiloGlobal
          ? parseFloat(purchase?.totalKiloGlobal)
          : 0,
        isHaveOrdered: purchase?.isHaveOrdered
      };
    });

    postPurchaseRices(formattedBody, {
      onSuccess: async () => {
  ...
  const { data, isFetching } = useGetAdminCustomerPurchaseRices(params, {
    staleTime: Infinity
  });
  ...
  useEffect(() => {
    ...
    const key = queries.adminCustomerPurchaseRices.list(params).queryKey;
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries(queries.adminCustomerPurchaseRices._def);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
          ...
          <TableBody>
            {isFetching ? (
              ...
              purchaseList?.map((row, idxPurchase) => {
                ...
                return displayedRow?.items?.map((_, idx) => {
                  return (
                    <MenuRow
